...
export const imageFile = async (folderName: string, fileUri: string, isImage: boolean = true) => {
    try {
        ...
        return { success: true, msg: 'image uploaded', data: publicUrlData.publicUrl };
...
export const getFilePath = (folderName: string, isImage: boolean = true) => {
    const fileExtension = isImage ? '.jpg' : '.txt';
