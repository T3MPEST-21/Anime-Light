...
const SkeletonCard = ({ theme }: { theme: any }) => (
  ...
  >
    <View style={styles.cardHeader}>
      <View
        style={[styles.skeletonAvatar, { backgroundColor: theme.skeleton }]}
      />
      <View style={{ flex: 1 }}>
        <View
          style={[styles.skeletonUsername, { backgroundColor: theme.skeleton }]}
        />
        <View
          style={[styles.skeletonTime, { backgroundColor: theme.skeleton }]}
        />
      </View>
      <View
        style={[styles.skeletonMoreButton, { backgroundColor: theme.skeleton }]}
      />
    </View>

    <View style={[styles.skeletonBody, { backgroundColor: theme.skeleton }]} />
    <View
      style={[styles.skeletonBodyShort, { backgroundColor: theme.skeleton }]}
    />
...
// Helper functions moved outside the component and exported
...
export default function Index() {
  ...
  const fetchPosts = useCallback(
    async (isRefresh = false, pageNum = 0) => {
      ...
      try {
        ...
        console.log("Fetch posts result:", {
          data,
          error,
        ...
        if (data && data.length > 0) {
          const postsWithProfiles = await Promise.all(
            data.map(async (post) => {
              ...
              console.log(`Images for post ${post.id}:`, {
                images,
                imagesError,
        ...
        if (e?.message?.includes("permission")) {
          Alert.alert(
            "Permission Error",
            "Unable to load posts. Please check your database permissions."
          );
        } else {
  ...
  const renderFooter = () => {
    ...
    return (
      <View
        style={[styles.footerLoader, { backgroundColor: theme.background }]}
      >
